...
export default {
  ...
  data() {
    return {
      ...
      options: {
        ...
        chunkSize: '20480000',   //分块大小
        fileParameterName: 'file', //上传文件时文件的参数名，默认file
        maxChunkRetries: 3,  //最大自动失败重试上传次数
        testChunks: true,     //是否开启服务器分片校验
        forceChunkSize:true,  //20M每块
        simultaneousUploads: 3,
        // 服务器分片校验函数，秒传及断点续传基础
        checkChunkUploadedByResponse: function (chunk, message) {
           let objMessage = JSON.parse(message);
            console.log("分片校验结果====="+objMessage)
            if(objMessage.data=='该文件块已经上传'){    
              return true;
            }
  ...
  methods: {
    onFileAdded (file) {
      console.log("this.$store.state.folderId=="+this.$store.state.folderId);
    ...
    onFileSuccess () {
      const parentIdStr=window.localStorage.getItem('folder');
      console.log("parentIdStr====="+parentIdStr);

      let url=window.location.href;

      let userName=null;
      if(url.indexOf("folder2")!=-1){
